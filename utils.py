...
def loadData(name):
    ...
    if name == 'IP':
        ...
        data = sio.loadmat(os.path.join(data_path, 'Augsburg.mat'))['Augsburg']
        labels = sio.loadmat(os.path.join(data_path, 'Augsburg_gt.mat'))['Augsburg_gt']
    elif name == 'LK':
        ...
        data = sio.loadmat(os.path.join(data_path, 'kansas.mat'))['kansas']
        labels = sio.loadmat(os.path.join(data_path, 'kansas_gt.mat'))['kansas_gt']
    elif name == 'XZ':
...
def splitTrainTestSet(X, y, Train_ratio, randomState=345):
    X_train, X_test, y_train, y_test = train_test_split(X, y, train_size=Train_ratio, random_state=randomState,
                                                        stratify=y)
